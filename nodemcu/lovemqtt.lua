...
local function check_sequence(button)
	local position = 1
	local updated_button = button
	while true do 
...
local function buzz_sequence(sequence_string)
...
local function publish_love(c,msg)
...
local function publish_node(c,msg)
...
local function nodeSubscription(client)
	local msgsrec = 0
...
local function conectado(client)
	client:subscribe(node_settings.subscribe, 0, nodeSubscription)
	
	local co = coroutine.create(check_sequence)
